...
if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    ...
    if (array_key_exists("employeeId", $_GET)) {
        ...
        if ($salaryHistory->salary_history_employee_id === "0") {

            //check to see if task id in query string is not empty and is number, if not return json error
            checkId($salaryHistory->salary_history_employee_id);
            $query = checkReadReportSalaryHistoryAllEmployee($salaryHistory);
            http_response_code(200);
            getQueriedData($query);
        }
        //check to see if task id in query string is not empty and is number, if not return json error
        checkId($salaryHistory->salary_history_employee_id);
        $query = checkReadReportSalaryHistoryByEmployeeId($salaryHistory);
        http_response_code(200);
        getQueriedData($query);
    }
