...
const ModalEditManageEarnings = ({ item }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        `${devApiUrl}/v1/earnings/update-amount/${item.earnings_aid}`,
        "put",
        values
      ),
    onSuccess: (data) => {
        ...
        <div className="p-1 w-[350px] rounded-b-2xl">
          <div className="flex justify-between items-center bg-primary p-3 rounded-t-2xl">
            <h3 className="text-white text-sm">
              Edit Earnings : {item.earnings_payroll_id}
            </h3>
            <button
              type="button"
          ...
          <div className="bg-white rounded-b-2xl overflow-hidden">
            <Formik
              ...
              onSubmit={async (values, { setSubmitting, resetForm }) => {
                // console.log(values);
                const earnings_amount = removeComma(
                  `${values.earnings_amount}`
                );
                mutation.mutate({ ...values, earnings_amount });
              }}
            >
              {(props) => {
                return (
                  <Form>
                    <div className="max-h-[28rem] overflow-y-auto p-4">
                      <div className="grid grid-cols-[6rem_1fr]">
                        ...
                        <p className="m-0">{item.earnings_payroll_id}</p>
                      </div>
                      <div className="grid grid-cols-[6rem_1fr]">
                        <p className="m-0 text-primary font-bold">
                          Employee :{" "}
                        </p>
                        <p className="m-0">{item.earnings_employee}</p>
                      </div>
                      ...
                      <div className="grid grid-cols-[6rem_1fr] ">
                        ...
                        <p className="m-0">{item.earnings_details}</p>
                      </div>
...
export default ModalEditManageEarnings;
